...
export default function CategoryChart({ data }) {
  ...
  const maxValue = Math.max(...categories.map(c => c.total_revenue || 0), 1);
        ...
        <div className="absolute inset-0 flex flex-col justify-around px-4">
          {categories.slice(0, 5).map((category, idx) => (
            <div key={idx} className="flex items-center gap-4">
              <span className="text-sm text-gray-600 w-28 truncate">{category.category}</span>
              <div className="flex-1 h-8 bg-gray-100 rounded-full overflow-hidden">
                <div 
                  className="h-full bg-info-500 rounded-full transition-all duration-500"
                  style={{ width: `${(category.total_revenue / maxValue) * 100}%` }}
                />
              </div>
