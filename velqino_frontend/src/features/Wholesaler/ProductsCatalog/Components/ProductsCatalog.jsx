...
const handleSearch = useCallback((value) => {
  ...
  const getStockText = (stock, threshold) => {
    ...
    // Single Edit - Opens modal
    const handleEditProduct = (product) => {
    console.log('Edit product:', product)
    toast.info(`Edit ${product.name}`)
    // Call parent's edit function if passed as prop
    if (onEditProduct) {
      onEditProduct(product)
    }
  }

    // Bulk Edit - Opens bulk edit modal
  ...
  setShowBulkEditModal(true)
    ...
    const handleViewProduct = (product) => {
      ...
      toast.info(`View ${product.name} - Implement view modal`)
    }
  ...
  // Vendor filter
  const matchesVendor = !vendorFilter || product.brand?.toLowerCase().includes(vendorFilter.toLowerCase())
  
  return matchesSearch && matchesCategory && matchesStockStatus && matchesMinPrice && matchesMaxPrice && matchesVendor
})
  ...
  <div className="products-catalog">
      ...
      {showFilters && (
        <div className="products-filters-panel mt-4 pt-4 border-t border-gray-200">
          ...
          <div className="flex items-center justify-end gap-2 mt-4">
            ...
            <button 
              onClick={() => {
                // Filters are already applied via state, just close panel if needed
    ...
    <div className="bg-primary-50 border border-primary-200 rounded-lg p-3 mb-4 flex items-center justify-between">
      ...
      <div className="flex items-center gap-2">
        <button 
          ...
          className="px-3 py-1.5 text-sm bg-white border border-primary-200 text-primary-700 rounded-lg hover:bg-primary-100 transition-all"
        >
          Bulk Edit
        </button>
        <button 
    ...
    {totalPages > 1 && (
      <div className="flex items-center justify-between mt-6 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <button 
            className="px-3 py-1.5 text-sm border border-gray-200 rounded-lg hover:bg-gray-50 disabled:opacity-50 transition-all"
